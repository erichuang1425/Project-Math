...
7. `chain-recip-graph`: implemented with authored samples for `y = 1/(x² + 1)` and its derivative `y = ??x/(x² + 1)²`, the maximum point `(0, 1)`, labeled axes, ticks, and a chain-rule annotation at the maximum.
...
- Do not create or update `docs/agent/WORK_PROMPT_QUEUE.md`.
- Keep the durable fresh-thread prompt in `docs/agent/NEXT_WORK_PROMPT.md`.
